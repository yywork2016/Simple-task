...
class FileModel extends Model{
     ...
     */
    public function downLocalFile($file, $callback = null, $args = null,$del=null){
        if(is_file($file['rootpath'].$file['savepath'].$file['savename'])){
            /* 调用回调函数新增下载数 */
            is_callable($callback) && call_user_func($callback, $args);
            if($del==1){
                $filePath=$file['rootpath'].$file['savepath'].$file['savename'];
                $fileDir=$file['rootpath'].$file['savepath'];
                $fp=fopen($filePath,"r");
            }
            /* 执行下载 */ //TODO: 大文件断点续传
            ...
            if($del==1){                
                ...
                //向浏览器返回数据 
                while(!feof($fp) && $file_count<$file['size']){ 
	...
	private function downFtpFile($file, $callback = null, $args = null){
		...
		header("Location:http://{$host}/index.php?key={$key}");
	}
